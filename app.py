"""
FurnaceGuard — AI Predictive Maintenance Dashboard
====================================================
Tata Steel Tandem Cold Rolling Mill #3 — Live Telemetry
Uses @st.fragment(run_every=...) for flicker-free partial reruns.
"""
...
with st.sidebar:
    ...
    st.slider("Vibration (mm/s)", 5.0, 25.0, 15.0, key="danger_vib")
    st.slider("Temperature (°C)", 60.0, 150.0, 100.0, key="danger_temp")
...
def live_dashboard():
    ...
    for i,(ic,lb,vl,un) in enumerate(kd):
        kpi_row[i].markdown(f'<div class="kpi"><div class="ic">{ic}</div><div class="lb">{lb}</div><div class="vl">{vl}</div><div class="un">{un}</div></div>', unsafe_allow_html=True)

    # --- AI ROW ---
    st.markdown('<div class="sec">🧠 AI Prediction & System Status</div>', unsafe_allow_html=True)
    ai_cols = st.columns([1.1, 1.4, 0.9, 0.9])
    ...
    ai_cols[0].plotly_chart(fg, use_container_width=True)

    # RUL
    ai_cols[1].markdown(f'<div class="rul-box"><div class="lb">Predicted Remaining Useful Life</div><div class="vl">{rul:.1f}</div><div class="un">Hours until maintenance required</div></div>', unsafe_allow_html=True)

    # Status
    current_level = "green" if rul > 72 else "amber" if rul >= 24 else "red"
    if current_level == "green":
        ai_cols[2].markdown('<div class="st-g"><div><span class="dot"></span><span class="tx">All Systems Nominal</span></div><div class="su">No action required</div></div>', unsafe_allow_html=True)
    elif current_level == "amber":
        ai_cols[2].markdown('<div class="st-a"><div><span class="dot"></span><span class="tx">Maintenance Soon</span></div><div class="su">Schedule inspection within 24h</div></div>', unsafe_allow_html=True)
        ...
        ai_cols[2].markdown('<div class="st-r"><div><span class="dot"></span><span class="tx">CRITICAL ALERT</span></div><div class="su">Immediate shutdown recommended</div></div>', unsafe_allow_html=True)
    ...
    ai_cols[3].markdown(f'<div class="roi"><div class="rl">Est. Downtime Savings</div><div class="rv">₹{savings:.1f}L</div><div class="rs">{avoided} critical failures predicted<br>Session: {uptime_min:.0f} min</div></div>', unsafe_allow_html=True)

    # --- CHARTS ---
    DANGER_VIB = st.session_state.danger_vib
    DANGER_TEMP = st.session_state.danger_temp

    def make_chart(df, col, title, color, fill_color, danger_val=None):
        f = go.Figure()
    ...
    st.markdown('<div class="sec">📈 Live Sensor Feeds — Trailing 50 Readings</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    c1.plotly_chart(make_chart(df_h, "Vibration_Velocity_mm_s", "Vibration Velocity (mm/s)", "#4da6ff", "rgba(77,166,255,0.06)", DANGER_VIB), use_container_width=True)
    c2.plotly_chart(make_chart(df_h, "Motor_Temperature_C", "Motor Temperature (°C)", "#FFBF00", "rgba(255,191,0,0.05)", DANGER_TEMP), use_container_width=True)
    c3, c4 = st.columns(2)
    ...
    if logs:
        ...
        st.markdown(f'<div class="gc"><table class="lt"><thead><tr><th>Time</th><th>Level</th><th>Detail</th></tr></thead><tbody>{rows}</tbody></table></div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="gc" style="text-align:center;padding:24px;color:#556">✅ No alerts — all systems operating within safe parameters</div>', unsafe_allow_html=True)
